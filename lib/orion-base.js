...
/*
var O = new Orion({ host: "localhost", port: 1026 }, new KS.Keystone({ host: "localhost", port: 35357 }));
O.keystone.auth = KS.Keystone.user_to_keystone_auth({ name: "giulio", password: "password" }, {name: "mivoqrecorder"}, { name: "/webrecorder" } );
O.keystone.token = null;
O.keystone.maxretry = 5;
O.default_error_handler = function(res) {console.log(JSON.stringify(res));};
console.log(O.keystone);
O.getEntity("Room1")
    .on('success', function(res) {console.log(JSON.stringify(res));})
    .on('error', function(res) {console.log(JSON.stringify(res));}).end();
*/
